...
class CityreaderTests(unittest.TestCase):
  ...
  def test_cityreader_stretch_correctness(self):
    ...
    #BUG - this is supposed to be an empty list, so length is gonna be 0
    inp = cityreader_stretch(32, -120, 45, -100, self.cities)

    #originally there was no re-defining of expected to change it from the list on line 18
    #FIX 
    expected = []

    #so this line would fail because it was checking the len of the line 38 inp(which is supposed to be 0) against the line 18 expected
    self.assertEqual(len(inp), len(expected))
